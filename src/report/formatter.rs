use super::schema::{AuditReport, IssueType, Severity};
...
pub fn print_human(report: &AuditReport) {
    ...
    let reality_gaps: Vec<_> = report
        .issues
        .iter()
        .filter(|i| {
            matches!(
                i.issue_type,
                IssueType::ContractViolation | IssueType::FakeWiring
            )
        })
        .collect();

    let dead_code: Vec<_> = report
        .issues
        .iter()
        .filter(|i| matches!(i.issue_type, IssueType::DeadCode))
        .collect();

    let slop: Vec<_> = report
        .issues
        .iter()
        .filter(|i| matches!(i.issue_type, IssueType::AiSlop))
        .collect();

    let exec: Vec<_> = report
        .issues
        .iter()
    ...
    let diagnostics: Vec<_> = report
        .issues
        .iter()
        .filter(|i| matches!(i.issue_type, IssueType::CompilerDiagnostic))
        .collect();
    ...
    if !diagnostics.is_empty() {
        println!("{}", "  ◆ COMPILER / TYPE DIAGNOSTICS".yellow().bold());
        println!(
            "{}",
            "  ─────────────────────────────────────────────────".dimmed()
        );
    ...
    if report.high_count() > 0 {
        println!(
            "{}",
            "  ✗ VERDICT: Code has critical structural issues."
                .red()
                .bold()
        );
        ...
        println!(
            "{}",
            "  ⚠ VERDICT: Code has wiring issues. Review before shipping."
                .yellow()
                .bold()
        );
    } else {
        println!(
            "{}",
            "  ✓ VERDICT: No critical issues found.".green().bold()
        );
    }
    println!(
        "{}",
...
pub fn print_json(report: &AuditReport) -> anyhow::Result<()> {
    println!(
        "{}",
        serde_json::to_string_pretty(&report.to_json_report())?
    );
    Ok(())
}
